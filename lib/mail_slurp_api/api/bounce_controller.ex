...
defmodule MailSlurpAPI.Api.BounceController do
  ...
  {:ok, %MailSlurpAPI.Model.PageListUnsubscribeRecipients{}} on success
  {:error, info} on failure
  """
  @spec get_list_unsubscribe_recipients(Tesla.Env.client, keyword()) :: {:ok, MailSlurpAPI.Model.PageListUnsubscribeRecipients.t} | {:error, Tesla.Env.t}
  def get_list_unsubscribe_recipients(connection, opts \\ []) do
    optional_params = %{
      :"page" => :query,
      :"size" => :query,
      :"sort" => :query,
    ...
    %{}
    |> method(:get)
    |> url("/bounce/list-unsubscribe-recipients")
    |> add_optional_params(optional_params, opts)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> evaluate_response([
      { 200, %MailSlurpAPI.Model.PageListUnsubscribeRecipients{}}
    ])
  end
end
